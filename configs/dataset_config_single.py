single_paths = ["/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/4dgs_dataset/coffee_martini",
                "/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/4dgs_dataset/cook_spinach/",
                "/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/4dgs_dataset/flame_steak/",
                "/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/4dgs_dataset/flame_salmon_1/",
                "/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/SelfCap/yoga-release/videos",
                "/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/google_lightfield/06_Goats_rect",  # rectified
                "/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/google_lightfield/01_Welder_rect", # rectified
                "/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/google_lightfield/02_Flames_rect", # rectified
                "/media/emmahaidacher/Volume/DATASETS/INTERNET_DATASETS/google_lightfield/07_Car_rect",    # rectified
                "/media/emmahaidacher/Volume/DATASETS/OWN_DATASETS/harp/synched1/only_videos"
                ]

single_names = ["coffee", "spinach", "steak", "salmon", "yoga", "goats", "welder", "flames", "car", "harp"]

single_crops = ["bottom", "bottom", "bottom", "bottom",  "bottom", "center", "center", "center", "center", "center"]

single_ss = [2, 5, 1, 11, 15, 0, 10, 4, 2, 15] # starting second

single_video_names = [["cam05.mp4", "cam00.mp4", "cam06.mp4"],     # coffee
                      ["cam00.mp4", "cam05.mp4", "cam04.mp4"],     # spinach
                      ["cam00.mp4", "cam06.mp4", "cam08.mp4"],     # steak
                      ["cam05.mp4", "cam00.mp4", "cam06.mp4"],     # salmon
                      ["0005.mp4", "0010.mp4"],                    # yoga
                      ["camera_0001.mp4", "camera_0006.mp4"],      # goats
                      ["camera_0001.mp4", "camera_0006.mp4"],      # welder
                      ["camera_0006.mp4", "camera_0001.mp4"],      # flames
                      ["camera_0006.mp4", "camera_0001.mp4"],      # car
                      ["4.mp4", "3.mp4"]                           # harp
                      ]

#               -coffee-    -spinach-   -steak-   -salmon-   -yoga--   -goats-   -welder-   -flames-   -car--   -harp--
#               -right--    -right---   -right-   -right--   -right-   -left--   -left---   -right--   -left-   -right-
single_trajs = [[6, 12],    [3, 6],     [4, 8],   [-4, -8],  [5],      [-3],     [-4],      [6],       [-5],    [5]]

single_distances = ["near", "middle", "far", "v_far", "vv_far"]
